...
export default function ProfilePanel({
    ...
    const uploadAvatar = async (file: File) => {
        ...
        await onUpdateProfile({ avatarUrl: data.avatarUrl });
    };
    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                <div style={{ position: "relative" }}>
                    <div style={{
                        ...
                        background: theme.border, display: "flex", alignItems: "center", justifyContent: "center",
                    }}>
                        {form.avatarUrl ? (
                            <img src={form.avatarUrl} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                        ) : (
                            <span style={{ color: theme.text, fontSize: 26 }}>
                ...
                <div>
                    <p style={{ margin: 0, fontSize: 15, fontWeight: "bold", letterSpacing: 1,
                        color: selectedProfile ? "#4ecdc4" : theme.text }}>
                        {profileData.displayName || profileData.username}
                    </p>
                    <p style={{ margin: 0, fontSize: 11, color: theme.textDim, letterSpacing: 1 }}>
            ...
            {!selectedProfile && !editing && (
                <button onClick={() => setEditing(true)} style={{ ...s.btnSmall, marginTop: 16, width: "100%" }}>
                    Editar perfil
                </button>
            )}
            ...
            {!selectedProfile && editing && (
                <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 16 }}>
                    ...
                    <select style={s.input} value={form.country}
                    ...
                    <DatePicker
                        ...
                        onChange={(date: Date | null) => setForm({ ...form, birthDate: date })}
                        dateFormat="dd/MM/yyyy" showMonthDropdown showYearDropdown dropdownMode="select"
                        minDate={new Date("1940-01-01")} maxDate={new Date()}
                        placeholderText="Fecha de nacimiento" className="custom-datepicker"
                    />
                    <button style={{ ...s.btn, ...s.btnPrimary }} onClick={saveProfile}>Guardar</button>
                    <button style={s.btnLink} onClick={() => setEditing(false)}>Cancelar</button>
                </div>
            )}
